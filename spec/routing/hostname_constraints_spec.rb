...
describe "ルーティング" do
  example "職員トップページ" do
    ...
    url = "https://#{config[:staff][:host]}/#{config[:staff][:path]}"
  ...
  example "管理者ログインフォーム" do
    config = Rails.application.config.baukis2
    url = "https://#{config[:admin][:host]}/#{config[:admin][:path]}/login"
    expect(get: url).to route_to(
  ...
  example "顧客トップページ" do
    config = Rails.application.config.baukis2
    url = "https://#{config[:customer][:host]}/#{config[:customer][:path]}"
    expect(get: url).to route_to(
  ...
  example "ホスト名が対象外ならroutableではない" do
    expect(get: "http://foo.example.jp").to_not be_routable
  end
